...
public class ShelterCardRestControllerTests {
        ...
        void cantUpdateShelterCard_NotFound() throws Exception {
                ...
                when(shelterCardService.getShelterCardById(nonExistentShelterCardId))
                                .thenThrow(ResourceNotFoundException.class);
                when(shelterCardService.save(shelterCard1)).thenReturn(shelterCard1);

                String shelterCard3JsonString = objectMapper.writeValueAsString(shelterCard1);

                MockHttpServletRequestBuilder requestBuilder = put("/api/v1/shelterCards/{id}",
                                nonExistentShelterCardId)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(shelterCard3JsonString)
                                .with(csrf());
        ...
        @Test
        @WithMockUser("PLAYER")
        void canDeleteShelterCardByGameId() throws Exception {
                Integer gameId = 1;
                Game game1 = new Game();
                game1.setId(gameId);

                ShelterCard ShelterCardTest = new ShelterCard();
                ShelterCardTest.setId(3);
                ShelterCardTest.setGame(game1);

                when(shelterCardService.getShelterCardByGameId(gameId)).thenReturn(List.of(ShelterCardTest));
                doNothing().when(shelterCardService).deleteByGameId(gameId);

                MockHttpServletRequestBuilder requestBuilder = delete("/api/v1/shelterCards?gameid={gameId}", gameId)
                                .contentType(MediaType.APPLICATION_JSON)
                                .with(csrf());
        ...
        @Test
        @WithMockUser("PLAYER")
        void cantDeleteShelterCardByGameId() throws Exception {

                Integer nonExistendShelterCardId = 33;

                when(shelterCardService.getShelterCardByGameId(nonExistendShelterCardId)).thenReturn(List.of());
                doNothing().when(shelterCardService).deleteByGameId(nonExistendShelterCardId);

                MockHttpServletRequestBuilder requestBuilder = delete("/api/v1/shelterCards?gameid={nonExistendGameId}", nonExistendShelterCardId)
                                .contentType(MediaType.APPLICATION_JSON)
                                .with(csrf());
